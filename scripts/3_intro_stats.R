...
example_data = rnorm(32, mean(mtcars$wt), sd(mtcars$wt)) #pull 32 points from random norm dist that has same mean and stdev as mtcars$wt (simulate perfect normal distribution)
hist(example_data)
qqnorm(example_data)
...
# As car weight increases, the Miles/(US) gallon tends to decrease. There is evidence for a significant negative linear correlation between these two variables (p-val: 1.294e-10).
...
augmented2_car_mod <- augment(car_mod, interval = 'prediction') # need to specify the optional argument interval here, otherwise defaults to not giving CI
glimpse(augmented2_car_mod)
ggplot(augmented2_car_mod, aes(x = wt, y = mpg)) + 
        geom_point() + 
        geom_line(aes(y = .fitted)) +  # augment uses .whatever for all the new columns it creates to avoid confusion with existing variables
...
#Does our car weight predict mpg? The car weight (1000 lbs) appears to predict Miles/(US) gallon.
#Even if a car weighed ~0 lbs, it would still require a certain amount of gas to run - it would be at least 37 mpg.
...
# I think that the setosa iris species may on average have the greatest sepal width. Possible rank may be versicolor < virginica < setosa.
# second, run model
...
# third, check assumptions
plot(iris_mod)
# Q: can you interpret these plots with respect to the assumptions of anova? No, it's not helpful for categorical variables
...
irischeck <- group_by(iris,Species)
summarize(irischeck, msw = mean(Sepal.Width))
...
ggplot(flowers, aes(x = colour, y = number.of.visits, color = size)) +
        geom_point() +
        theme_bw()
# Q: Can you eyeball-check any anova assumptions here? no, there are probably overlapping datapoints
...
#Both flower color and size predict the number of visits. Additionally, there is an interaction between these variables because
#how the number of visits (y) varies with flower color (x) depends on the level of size.
...
# Q: for each plot, write down what you would conclude
        #The distribution of the residuals appears relatively normally distributed.
        #Homoscedasticity - The variance of the residuals is relatively similar across the fitted values, however the residuals are 
        #relatively higher on either end of the distribution (when fitted values are low/high).
        #There do not appear to be any major outliers (Cook's Distance for Chrysler Imperial ~0.5).
...
# Q: write the model equation for egg production in the summer in y = a + bx form, filling in all coefficients and variable names
        # EGGS(summer) = 2.66 - 0.81 - 0.03(Density)
...
means(5,10000)
means(50,10000)
means(500,10000) #narrowest distribution about 0
# Q: then run means() with sample_size = 50 and number_samples = 10, then number_samples = 1000, then = 10,000
means(50,10) #low frequency
means(50,1000) 
means(50,10000) #high frequency
