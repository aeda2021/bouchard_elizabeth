...
hist(rbinom(500,10,0.01)) #500 observations, 10 trials (coin flips), probability of 0.01
hist(rbinom(500,10,0.9))
...
ggplot(data=soay, aes(x = body.size, y = fitness)) +
  geom_point()+
 # geom_smooth()+
...
# Q: write code below to run the lm model and look at the diagnostics, including a histogram of the residualss
sheep_mod <- lm (fitness ~ body.size, data = soay)
hist(sheep_mod$residuals) # first look at a histogram of the residuals
autoplot(sheep_mod) # autoplot is the ggplot function that automatically generates assumption-checking pictures for you
...
hist(soay$fitness)
mean(soay$fitness)
# Q: why might we be in poisson glm territory here?   #Peak near zero. Quite a few zeros and low mean.
...
# this is different from what we are used to thinking about with linear fits, where y increases by a constant absolute amount 
# for each unit change in x (e.g., y=2x always changes by 2 per unit change in x, regardless of where you are on the graph)
...
# odds of 27:1 means that out of 28 women on the titanic, 27 are expected to survive
# 27/28 = 0.96
...
odds / (1 + odds)	 #survival probability of ~0.8
...
odds #survival odds of 0.1074639:1
odds / (1 + odds)	 #survival probability of ~0.09703606
