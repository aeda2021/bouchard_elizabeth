...
#First, I'm checking the assumptions.
hist(mod1$residuals) #histogram of residuals
autoplot(mod1, smooth.colour = NA)
#The residual vs. fitted plot has a pattern that looks like a hill, which indicates that 
#a line is an appropriate fit to this data. In the Q-Q plot, the points deviate from the
#line on the left and right sides, suggesting that it may violate the assumption that the 
#residuals are normally distributed. The histogram of residuals also appears skewed to the 
#right. In the scale-location plot, the vertical spread of the points varies across the 
#fitted values, which violates the assumption that variance of Y is similar across X.
#There are points in the lower right corner of the residuals vs. leverage plot, which violates
#the assumption that the results do not depend on a small proportion of the data.
#In my opinion, the data violate the assumptions of the linear model, so it would not be 
#appropriate to use a linear model here.
...
#The cut() function breaks the x values into groups based on whatever breaks you specify and labels it.
#Here, we used the cut() function to identify which elements were in the low half versus the high half
#of the data based on sample depth. We labeled them as 1 or 2. X values below 1589.5 were assigned a 1 and 
#x values above 1589.5 were assigned a 2. Then, we saved it (ISITsub$split2).
...
#The fit between the model and the data has improved. The downside to continuing with this method is that 
#each time you split the data you reduce the sample size, thus increasing the uncertainty.
...
#The 1-model fit is furthest from the observed data. The 10-model fit has the widest confidence bounds.
#This illustrates the bias-variance trade-off because the 1-model is biased with relatively low variance
#whereas the 10-model fit is less biased but it has high variance.
...
#I think cod prefer colder water because there were more presence occurrences in the north (i.e. New England
#and Canada) compared to the south (i.e. the Gulf up to the mid-Atlantic).
...
ggplot(spdata, aes(x=SBT.actual, y=presfit01)) +
    geom_point()
#Cod have been observed at temperatures between about 0 and 18 degrees celsius.
...
modg <- gam(presfit01 ~ s(SBT.actual), data = spdata)
gam.check(modg)
#The assumptions have not been met because:
    #1) the points do not fall along the 1-to-1 line in the deviance residuals vs. theoretical quantiles plot,
        #indicating that the residuals are not normally distributed.
    #2) the residuals vs. linear predictor plot has a strong linear pattern,
        #indicating that the residuals are not evenly distributed about the linear predictor.
    #3) the histogram of residuals also confirms that the residuals are not normally distributed.
    #4) the response vs. fitted values plot is binomial. 
...
#I chose to use binomial because the data consists of 0s and 1s.
...
gam.check(mod2)
#It looks like the assumptions have been met to an extent. The assumptions for mod2 are met to a greater extent than modg, but they are not perfect. 
#Full convergence was achieved after 11 iterations and the Hessian was positive definite.
#The p-value is 0.12, indicating that k is not too low.
#In the deviance residuals vs. theoretical quantiles plot, the points fall closer to the 1-to-1 line, indicating that the residuals are more normally distributed.
#The residuals vs. linear predictor plot has a strong linear pattern. This does not meet the assumption that the residuals are evenly distributed about the linear predictor. 
#The histogram of residuals is bimodal, which violates the assumption that the residuals are normally distribution. However, the distribution appears more similar to a normal
#distribution than the corresponding plot for modg.
#Due to the binomial data, the plot of response vs. fitted values is difficult to interpret. Typically, if the points fall along a straight line, it would 
#indicate that the model explains much of the variation in the data. 
...
summary(mod2)
#The SBT.actual term is significant at alpha=0.05 because the p-value is less than 2e-16.
#The estimated degrees of freedom (edf) is 8.959, so I expect the smooth fit to be quite wiggly.
#About 26.5% of the deviance is explained.
...
#The fit does look realistic because it shows that cod are more likely to be present in colder waters, as suggested by our initial map.
#I think it could be overfit because there bimodal peaks. I would expect cod to have an optimal temperature range, which would resemble a unimodal curve
#because their thermal tolerance would decline away from that temperature.
...
modsurv <- gam(presfit01 ~ s(SBT.actual) + factor(region), data = spdata, family=binomial)
gam.check(modsurv) #Hessian positive definite, p-value is 0.61
AIC(mod2,modsurv)
#Based on the AIC scores, I would choose the model that includes a categorical predictor for survey because it has the lower AIC score.
#This model has an AIC score of ~8315 whereas the model that only includes SBT has an AIC score of about 9116. The deltaAIC is greater than 
#10, providing substantial support that the model that includes survey is the better model given our data. As a result, I am confident stating that
#this (modsurv) is the better model.
