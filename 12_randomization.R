...
# lastly bootstrapping is useful when your sample size is too small for traditional stat methods (rule of thumb, n<30).
...
#The distributions of the original sample and the bootstrapped sample are quite similar. They have the same mode and they are both right skewed.

# Q: do the two distributions have the same mean? write code below to find out

mean(pennies_sample$year) #1994.82
mean(pennies_resample$year) #1993.58

#The means of the two distributions are very similar. The mean for the original pennies sample is 1994.82, whereas the mean for the resampled distribution is 1993.58.

# Q: what would happen if you sampled without replacement?

#If you sampled without replacement, then the resampled distribution would be identical to the original distribution.
...
#You can get a pretty spread-out distribution of means by re sampling a single sample because you are sampling with replacement.
#This allows the mean to fluctuate among different re samples. For example, the mean may be low if 1990 was selected many times in one re sample,
#whereas the mean could be high if 2020 was selected many times. Some values will be over-represented while others will be under-represented.
...
#I expect that as we decrease the number of reps, the distribution of the means will more poorly approximate the sampling distribution of the mean.
#In other words, the distribution will be more random.

pennies_resample2 <- pennies_sample %>% 
  rep_sample_n(size = 50, replace = TRUE, reps = 20)
fewMean <- pennies_resample2 %>%
  group_by(replicate) %>%
  summarize(meanYear = mean(year))

hist(fewMean$meanYear, breaks = 30)
...
pennies_resample3 <- pennies_sample %>% 
  rep_sample_n(size = 20, replace = TRUE, reps = 100)

low_n_Mean <- pennies_resample3 %>%
  group_by(replicate) %>%
  summarize(meanYear = mean(year))

hist(low_n_Mean$meanYear, breaks = 30)
...
ggplot(data = pennies_resample_medians, aes(x=median_year)) +
  geom_histogram(binwidth = 1, color = "white") +
  geom_vline(xintercept = CI_medians$l, color = "green") +
  geom_vline(xintercept = CI_medians$u, color = "green")
...
# other than that the data points are independent of each other and are an unbiased sample from the population
...
#The data is not tidy because Year is not a separate column. Instead, year is combined with the number of bees. 
#They don't need to be tidy.
...
#Sample does a permutation if you set replace to F because permutations do not use replacement. They simply rearrange the data within each column.
...
#Yes, column sums are maintained. The column sums should be maintained because permutations do not use replacement, so the same values should be in each column both
#before and after the permutation is performed.

rowSums(bee_prefs)
rowSums(bee_prefs_perm)
# Q: are row sums maintained?  should they be?

#Row sums are not maintained. Row sums should not be maintained because permutation is reordering the values within each column.

# Q: write code below to do a permutation similar to that above,
# but now permute the rows instead of the columns

bee_prefs_perm_row = matrix(0,15,4)
bee_prefs_perm_row
for (i in 1:15) {
  bee_prefs_perm_row[i,] = sample(bee_prefs[i,], replace=F)		
}
bee_prefs_perm_row

colSums(bee_prefs)
colSums(bee_prefs_perm_row)
rowSums(bee_prefs)
rowSums(bee_prefs_perm_row)
...
# 1) Takes one random sample from the visits column in the visits dataframe and calls it visit_draw.
#This is the number of visits to a site on a particular day. It is an integer value.
# 2) Takes random sample from consp_grains column of pollen dataframe and names it pollen_draws. The number of random samples it takes will be determined by the random number
#drawn in step one. This will be the number of pollen grains deposited on one flower during a bee visit. There will be one value for each visit as determined by the number
#drawn in step one.
# 3) Sums the values in pollen_draws to give the number of pollen grains deposited on one flower during all bee visits to that site on a particular date.
# 4) These three steps are repeated 10000 times.

# Q: why did we use size=visit_draw?

#We used size=visit_draw because we use the number of bee visits to a particular site on a particular day (visit_draw) to decide how many random samples
#to pull from conspec_grains in the pollen dataset. conspec_grains is the number of conspecific pollen grains deposited on the stigma of one flower 
#during one bee visit.
...
mean_vis
hist(rpois(173, mean_vis), breaks = 50) #eliz: our above histogram is more skewed than this simulated poisson distribution
...
#This distribution looks more similar to our data than the poisson distribution.
...
#The gamma distribution is the best fit because it has the lowest AIC value (857.1064). 

# Q: write code below to choose a distribution for the pollen data

hist(pollen$consp_grains, breaks = 50) #distribution of pollen data

mean_pol <- mean(pollen$consp_grains)
mean_pol
hist(rpois(173, mean_pol), breaks = 50) #simulated poisson distribution
# looks like our pollen grain data are too skewed for a poisson
# too many zeros and too many really large values
...
scale1
hist(rgamma(173, shape=shape1, scale=scale1), breaks = 50) #simulated gamma distribution
#The gamma distribution seems to do a better job for our pollen data.

pollen_pois <- fitdist(pollen$consp_grains, "pois")
summary(pollen_pois) # summary gives AIC for the fit, just like for a model fit
pollen_gamma <- fitdist(pollen$consp_grains, "gamma")
summary(pollen_gamma)  # hmm this also got somewhat different parameter values using maximum likelihood, than we did using the formula
...
# use "independence" if you want to ask whether two variables are independent
...
#R just returned the mean dry mass of each replicate.
...
mean(bee_mass$dry_mass_mg) #29.73849
mean(b_mean$stat) #29.73133
...
#The quantities in the output column 'stat' are the difference in means (mass) between the queen and worker for a specific replicate.

# Q: how will you use these numbers? what are we trying to test here?

#These numbers give you what you'd expect if bee mass and caste were independent, our null expectation.
#We will compare these numbers to the original data. We are trying to test whether dry mass is independent from bee caste (aka
#whether there is a difference in mass of queen and worker bees). The permutation distribution resembles what we'd expect if the two variables are independent.
...
#The mean of the replicates is 0.01764985. This null distribution suggests there is little difference in the means of the mass for queens and workers.
#I'd expect it to be about zero because the null expectation is that the dry bee mass and caste are independent.
...
#I expect the conclusion to be that queen bees have greater mass than worker bees.
...
#The bootstrapped results indicate that the mean bee mass is about 29.7 g, but the results of our permutation analysis suggest that mass may differ between
#queen and worker bees. We should incorporate this into our analysis by finding the confidence interval separately for queen and worker bees.
...
get_p_value(null_dist,obs_stat,direction="two-sided") #p value = 0.0502
