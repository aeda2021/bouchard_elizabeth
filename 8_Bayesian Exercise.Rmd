...
plotPost(fm.crabs.dat$beta_0, main = 'Intercept')
plotPost(fm.crabs.dat$beta_1, main = 'Effect of Salinity')

sapply(1:ncol(fm.crab.dat), function(x) {
plotPost(fm.crab.dat[,x], main = colnames(fm.crab.dat)[x])
})
...
tick.mod <- function(){
  for(i in 1:N){
    y[i] ~ dbern(p[i])
    logit(p[i]) <- b0[site[i]] + b1*x1[i] + b2*x2[i] + b3*x3[i]
  }
  ## Random intercepts:
   for(j in 1:n.sites){
    b0[j] ~ dnorm(alpha,1/sigma_site^2) 
  }
...
params <- c('alpha', 'b1', 'b2', 'b3', 'sig.site', 'sig.obs')
