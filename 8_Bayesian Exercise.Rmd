...
A marginal probability is the probability of that parameter across all values of the other parameters. In other words, a marginal probability is the probability of the given parameter irrespective of everything else. In the context of the model, the marginal probability is the probability distribution of a given parameter without regard for the other parameters, given the data and assumptions. 
...
[b0j, b1, e, alpha, variance | yij] is proportional to the product across all observations of norm(yij|b0j+b1x1,e) * norm(b0j|alpha,variance) * norm(b1|0,100) * norm(alpha|0,100) * gamma(variance|1,0.01)  * gamma(e|1,0.01)
...
*Question*: Draw the Bayesian network of this model, and write out the mathematical probability statement. For priors, use normal(mean = 0, sd = 10) on the intercept and beta coefficient.

[beta_0, beta_1 | yi] is proportional to the product across all observations of pois(yi|exp(beta_0 + beta_1 * X_1)) * norm(beta_0|0,10) * norm(beta_1|0,10)
...
Below, translate your probability statement into a JAGs model. Remember, for() loops are your product symbols. Use the priors described above.

```{r Try it - crab model}
crab.mod <- function(){
  for(i in 1:N){
    y[i] ~ dpois(lambda[i])
    log(lambda[i]) <- beta_0 + beta_1*x1[i]
  }
   # Priors:
  beta_0 ~ dnorm(0,0.01) # precision .01 = sd 10
  beta_1 ~ dnorm(0,0.01)
...
datList.crabs <- list(
 y = crabs$burrows, # number of observations
 N = length(crabs$burrows), # observations themselves
...
plotPost(fm.crabs.dat$beta_0, main = 'Intercept, or Mean Number of Burrows', xlim = c(2,3))
plotPost(fm.crabs.dat$beta_1, main = 'Effect of Salinity', xlim = c(-0.05,0.05))

#Taking log scale into account
plotPost(exp(fm.crabs.dat$beta_0), main = "Intercept, or Mean Number of Burrows")
plotPost(exp(fm.crabs.dat$beta_1), main = "Effect of salinity", xlim=c(1.0,1.1))
...
The mean (or expected) number of burrows is 11.07 and the 95% conference interval ranges from about 9.6 to 12.6 burrows. 
The intercept means that we would expect there to be about 11 burrows at the mean salinity level. If we hadn't centered salinity measurements, our interpretation of the intercept would be the number of expected burrows when the salinity is 0.
On average, the number of burrows increases by a factor of about 1.03 (95% confidence interval of 1.02 to 1.05) for every 1 unit increase in salinity.
Given our data and assumptions, the probability that salinity has no effect is less than 2.5%.
...
[b0j, b1, b2, b3, pij, alpha, variance | yij] is proportional to the product across all observations of bern(yij|logit(b0j+b1x1+b2x2+b3x3)) * norm(b0j|alpha,variance) * norm(b1|0,5) * norm(b2|0,5) * norm(b3|0,5) * norm(alpha|0,2) * gamma(variance|1,1)
...
tick.mod <- function(){
  ...
  ## Random intercepts:
   for(j in 1:n.sites){
    alpha[j] ~ dnorm(alpha0,1/sigma_site^2) 
  }
...
initial.values.ticks <- replicate(3, list(list(
  alpha0 = rnorm(1, 0, 2), # 0.5
  b1 = rnorm(1, 0, 5), # 0.2
  b2 = rnorm(1, 0, 5),
  b3 = rnorm(1, 0, 5),
  sigma_site = rgamma(1,1,0.01) #1
)))
  
params <- c('alpha0', 'b1', 'b2', 'b3', 'sigma_site')
...
Plot posteriors for EC occurrence, the effects of forest area, edge, and type, and the among-site variance in EC occurrence. Give interpretable titles to your plots. If you want to transform the intercept from log-odds scale to probability, use the plogis() function.
...
plotPost(plogis(fm.ticks.dat$alpha0), main = "Mean Site-level EC occurrence")
plotPost(plogis(fm.ticks.dat$b1), main = 'Effect of Forest Area')
plotPost(plogis(fm.ticks.dat$b2), main = 'Effect of Forest Edge')
plotPost(plogis(fm.ticks.dat$b3), main = 'Effect of Forest Type')
plotPost(plogis(fm.ticks.dat$sigma_site), main = "Among-site Variance in EC Occurrence")
...
*Answer:*
The probability of a plot testing positive for EC occupancy was about 0.27, with a 95% confidence interval of 0.139-0.423.
Forest area appears to have an effect on EC occupancy. For every one unit increase in forest area, you'd expect a 0.35 increase in the probability of EC occupancy (with a 95% confidence interval of 0.237 to 0.482).
Forest edge also appears to effect EC occupancy. For every one unit increase in forest edeg, you'd expect a 0.32 increase in  EC occupancy (with a 95% confidence interval of 0.195 to 0.438).
Forest type also appears to be important. For every one unit increase in the proportion of forest that is evergreen/coniferous, you'd expect a 0.485 increase in EC occupancy, with a 95% confidence interval of 0.374-0.591.
Overall, I would conclude that we expect large, fragmented, coniferous forests to have higher probabilities of detecting EC. The EC occurence also appears to differ among sites, as indicated by the among-site variance in EC Occurrence (mean is 0.75 with 95% confidence interval of 0.59-0.91).